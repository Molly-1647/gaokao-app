...
// 设备框（来自 skill assets/ios_frame.jsx，禁止手写刘海/状态栏）。
// 原样移植原型的 iosFrameStyles + IosFrame。
const iosFrameStyles = {
  wrapper: { display: 'inline-block', padding: 12, background: '#000', borderRadius: 60, boxShadow: '0 0 0 2px #1f2937, 0 24px 70px rgba(120,70,20,0.35)', position: 'relative' },
  ...
  wifiIcon: { width: 16, height: 12, position: 'relative' },
  ...
  homeIndicator: { position: 'absolute', bottom: 10, left: '50%', transform: 'translateX(-50%)', width: 140, height: 5, background: 'rgba(0,0,0,0.32)', borderRadius: 999, zIndex: 10 },
};

export function IosFrame({ children, width = 393, height = 852, time = '9:41', battery = 92 }) {
  const textColor = '#000';
  return (
    <div style={iosFrameStyles.wrapper}>
      <div style={{ ...iosFrameStyles.screen, width, height, background: '#fff' }}>
        <div style={{ ...iosFrameStyles.statusBar, color: textColor }}>
          ...
          <div style={iosFrameStyles.statusIcons}>
            <div style={iosFrameStyles.signalIcon}>
              <div style={{ ...iosFrameStyles.signalBar, height: 4 }} /><div style={{ ...iosFrameStyles.signalBar, height: 6 }} /><div style={{ ...iosFrameStyles.signalBar, height: 9 }} /><div style={{ ...iosFrameStyles.signalBar, height: 11 }} />
            </div>
